...
*/
	...
	int Evaluation_func(vector< vector<int> > state, vector<int> piece_count, int* weights){
	...
	string minValue(Node* node, int numb_ply, string s, int towers_at_risk_opp_old, int target_soldier_opp_old);

	string maxValue(Node* node, int numb_ply, string s, int towers_at_risk_opp_old, int target_soldier_opp_old){
		...
		for(int i=0; i<moves.size(); i++){
			...
			temp_string = minValue(&temp_node, numb_ply-1, moves[(random_ind+i)%(moves.size())], towers_at_risk_opp_old, target_soldier_opp_old);
	...
	string minValue(Node* node, int numb_ply, string s, int towers_at_risk_opp_old, int target_soldier_opp_old){
		...
		bool quescnece = false;
		if(moves.size()==0){
			cout<<"opponent has no move left \n";
		}
		if(numb_ply==0 || moves.size() == 0){
			//Quescence.........................................
			vector<int> target_tower_opp;
			int target_soldier_opp_now;
			if(player==1){
				target_soldier_opp_now = opp_soldiers_under_attack((*node).state,1,1);
				target_tower_opp = opp_tower_attacks_possible((*node).state,1,1);
			} 
			else{
				target_soldier_opp_now = opp_soldiers_under_attack((*node).state,1,2);
				target_tower_opp = opp_tower_attacks_possible((*node).state,1,2);
			}  
			int towers_at_risk_opp_now = 0;
			for(int i=0; i<(*node).state.size()/2; i++){
				if(target_tower_opp[i]>0)
					towers_at_risk_opp_now++;
			}
			if(moves.size() != 0 && (towers_at_risk_opp_now > towers_at_risk_opp_old)){
				cerr<<"Quescence tower attack it is !!"<<endl;
				numb_ply = 2;
				quescnece = true;
			}
			else if(moves.size() != 0 && (target_soldier_opp_now > target_soldier_opp_old)){
				cerr<<"Quescence solidier attack is !!"<<endl;
				numb_ply = 1;
				quescnece = true;
		...
		for(int i=0; i<moves.size(); i++){
			temp_node = *node;
			temp_node.state  = update_state(temp_node.state, moves[(random_ind+i)%(moves.size())], (temp_node.pieces_count));
			temp_string = maxValue(&temp_node, numb_ply-1, moves[(random_ind+i)%(moves.size())], towers_at_risk_opp_old, target_soldier_opp_old);
	...
	string miniMax(Node* node, int numb_ply){
		...
		//Quescence.....................
		vector<int> target_tower_opp;
		int target_soldier_opp;
		if(player==1){
			target_soldier_opp = opp_soldiers_under_attack((*node).state,1,1);
			target_tower_opp = opp_tower_attacks_possible((*node).state,1,1);
		}
		else {
			target_soldier_opp = opp_soldiers_under_attack((*node).state,1,2);
			target_tower_opp = opp_tower_attacks_possible((*node).state,1,2);
		}
		int towers_at_risk_opp = 0;
		for(int i=0; i<(*node).state.size()/2; i++){
		...
		//...............................
		string action = maxValue(node, numb_ply,"", towers_at_risk_opp, target_soldier_opp);
	...
	string myNextMove(Node* curr_node){
		...
		vector<string> valid_moves = getAllValidMoves((*curr_node).state,my_player, 1);
		 	if(move_count<20){			//move greedily
				...
				if(move_count>=20 && move_count<40){	//ply of 4
					cerr<<"3 ply"<<endl;
					next = miniMax(curr_node, 3);
				} 
...
/*
Quescence state is when -
1) Inc in opp tower under attack
*/
