...
int weights[14] = {500,1000,-500,-1000,300,900,-400, -1500,100, 300,350,-150, -300, -350};
int my_player=0;
int opp_player=0;
int player = 0; 
string last_move = "";
...
*/
	...
	string maxValue(Node* node, int numb_ply, string s){
		...
		for(int i=0; i<moves.size(); i++){
			...
			//cerr<<"MAX:................. \n";
			// cerr<<"Beta: "<<(*node).beta<<endl;
			// cerr<<"Alpha: "<<(*node).alpha<<endl;
		...
		// cerr << "MOve max: " << moves[action_index] << "Eval value" << max1 << endl;
	...
	string minValue(Node* node, int numb_ply, string s){
		...
		// cerr << "MOve min: " << moves[action_index] << "Eval value" << min1 << endl;
		return moves[action_index];
	}
	...
	string greedyBestStep(Node* node){   //state and pieces_count updated
		...
		for(int i=0; i<valid_moves.size(); i++){
			...
			// cerr << "MOve is: " << valid_moves[(random_ind+i)%(valid_moves.size())] << endl;
			temp_eval = Evaluation_func(branch_state, temp_pieces_count);
			if(temp_max <= temp_eval){
				temp_max = temp_eval;
				// cerr<<"temp max: "<<temp_max<<endl;
	...
	string myNextMove(Node* curr_node){
		string next = "";
		 	// if(move_count<6 || tower_danger((*curr_node).state, -1) || tower_danger((*curr_node).state,1)){			//move greedily
		 	if(move_count<6){			//move greedily

			 	next = greedyBestStep(curr_node);
			}
			else if(true){
				if(move_count>=6 && move_count<10){	//ply of 4
					// cerr<<"2 ply"<<endl;
					next = miniMax(curr_node, 10);
				} 
