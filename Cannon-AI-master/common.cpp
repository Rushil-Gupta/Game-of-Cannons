...
*/
	...
	vector<string> getValidMovesForSoldier(vector<vector<int> > board, int x, int y, int player,int val, vector<string> moves){
		if(player == 2){
			...
			if(x > 0 && y >0){
				if(board[x-1][y-1] != val && board[x-1][y-1] != (2 * val))
					moves.push_back("S " + to_string(x) + " " + to_string(y) + " M "+ to_string(x-1) + " " + to_string(y-1));
			}

			if(y >0){
				if(board[x][y-1] != val && board[x][y-1] != (2*val))
			...
			if(x+1 < board.size() && y-1 >= 0){
				if(board[x+1][y-1] == 0){
					...
					if(x+3 < board.size() && y-3 >= 0){
						if(board[x+3][y-3] != val && board[x+3][y-3] != (2*val))
		...
		if(check_cannon(x,y,val,"BV")){
			...
			if(y-1 >= 0){
				if(board[x][y-1] == 0){
					...
					if(y-3 >= 0){
						if(board[x][y-3] != val && board[x][y-3] != (2*val))
		...
		if(check_cannon(x,y,val,"RH")){
			...
			if(x-1 >= 0){
				if(board[x-1][y] == 0){
					if(x-2 >= 0){
						if(board[x-2][y] != val && board[x-2][y] != (2*val))
							moves.push_back("S " + to_string(x) + " " + to_string(y) + " B "+ to_string(x-2) + " " + to_string(y));			
					}
					if(x-3 >= 0){
						if(board[x-3][y] != val && board[x-3][y] != (2*val))
	...
	string myNextMove_8_8(Node* curr_node){
		string next = "";
	...
	string myNextMove_10_10(Node* curr_node){
		string next = "";
		vector<string> valid_moves = getAllValidMoves((*curr_node).state,my_player, 1);
		 	if(move_count<10){			//move greedily
				...
				else if(move_count>=10 && move_count<60){	//ply of 4
					cerr<<"2 ply"<<endl;
					next = miniMax(curr_node, 2);
				} 
				else if(move_count>=60){	//ply of 4
					cerr<<"4 ply"<<endl;
					next = miniMax(curr_node, 4);
				}
				// else {	//ply of 4
	...
	string myNextMove_10_8(Node* curr_node){
		string next = "";
		vector<string> valid_moves = getAllValidMoves((*curr_node).state,my_player, 1);
		 	if(move_count<8){			//move greedily
			 	// next = greedyBestStep(curr_node, weights);
				...
				else if(move_count>=8 && move_count<40){	//ply of 4
					cerr<<"4 ply"<<endl;
					next = miniMax(curr_node, 2);
				} 
				else if(move_count>=40 && move_count < 80){	//ply of 4
					cerr<<"6 ply"<<endl;
					next = miniMax(curr_node, 4);
				}
				else {	//ply of 4
	...
	void updateWeights_8(int new_score, int prev_score){
...
void updateWeights_10(int new_score, int prev_score){
	cerr << "New Score: " << new_score << " PUrana kya hai bhai: " << prev_score << endl;
	vector<int> thresh_high{1000,3500,-500,-3000,500,500,500,-100,-100,-100};
	vector<int> thresh_low{200,2000,-1000,-6000,100,100,100,-500,-500,-500};
	...
	if(pieces_count[0] <= 8 && update_soldier_weight){
		update_soldier_weight = false;
		weights[0] += 500;
	}
	if(pieces_count[3] < 5 && update_tower_weight){
		update_tower_weight  =false;
		weights[3] -= 500;
	}
	// if(pieces_count[1] < 4 && update_hori_weight){
	...
	int main(){
		...
		if(ID == 1){
			...
			while(1){
				if(n==8 && m==8)
					string action_to_perform = myNextMove_8_8(&start);
				else if(n==8 && m==10)
					string action_to_perform = myNextMove_10_8(&start);
				else if(n==10 && m==10)
					string action_to_perform = myNextMove_10_10(&start);
				...
				if(prev_score != INT_MAX && prev_score != score){
					cerr << "OKay sir!" << endl;
					if(n==8)
						updateWeights_8(score,prev_score);
					else
						updateWeights_10(score,prev_score);
				}
				prev_score = score;
				pieces_count = start.pieces_count;
				start.state = board;
